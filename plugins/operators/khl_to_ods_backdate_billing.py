...
class KHLToStagingDailyOperator(BaseOperator):
    ...
    def pre_execute(self, context):
        ...
        if context["dag_run"].run_type == "scheduled":
            ...
            self.start_time = context['data_interval_start'].subtract(days=1).in_tz('Asia/Ho_Chi_Minh').strftime('%Y-%m-%d %H:%M:%S')
            self.end_time = context['data_interval_end'].subtract(days=1).in_tz('Asia/Ho_Chi_Minh').strftime('%Y-%m-%d %H:%M:%S')
