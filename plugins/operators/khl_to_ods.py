...
class KHLToStagingDailyOperator(BaseOperator):
    ...
    def pre_execute(self, context):
        ...
        if context["dag_run"].run_type == "scheduled":
            run_hour = logical_date.hour
            run_minute = logical_date.minute

            if run_hour == 2 and run_minute == 1:
                self.start_time = base_date.strftime('%Y-%m-%d %H:%M:%S')
                self.end_time = base_date.replace(hour=12, minute=0, second=0).strftime('%Y-%m-%d %H:%M:%S')
            elif run_hour == 2 and run_minute == 31:
                self.start_time = base_date.replace(hour=12, minute=0, second=0).strftime('%Y-%m-%d %H:%M:%S')
