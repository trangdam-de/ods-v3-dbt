...
from plugins.operators.khl_to_ods_backdate_billing import KHLToStagingDailyOperator
...
with DAG(
        'khl_backdate_dag__billing_full',
    ...
    def should_process_table(table_name, **context):
        tables_to_process = ["khl_portal_billing_result"]
    ...
    def should_run_by_time(**context):
        ...
        for key, value in pipeline_params.items():
            with TaskGroup(group_id=key) as one_to_one_task:
                    check_time = ShortCircuitOperator(
                        task_id = f"check_time_{key}",
                        python_callable = should_run_by_time,
                    )
